...
class RollbackPaymentHandlerTest {
    ...
    @EnumSource(value = Event.class, names = {
        "MOVE_TO_PAYMENT_PENDING", "ROLLBACK_PAYMENT"
    })
    void should_setup_payment_status_as_failed(Event event) {
    ...
    @ParameterizedTest
    @EnumSource(value = Event.class, names = {
        "ROLLBACK_PAYMENT_TIMEOUT", "ROLLBACK_PAYMENT_TIMEOUT_TO_PAYMENT_PENDING"
    })
    void should_setup_payment_status_as_timeout(Event event) {
        ...
        assertNotNull(callbackResponse);
        verify(asylumCase).write(AsylumCaseFieldDefinition.PAYMENT_STATUS, PaymentStatus.TIMEOUT);
    }
            ...
            for (PreSubmitCallbackStage callbackStage : PreSubmitCallbackStage.values()) {
                ...
                if (callbackStage == PreSubmitCallbackStage.ABOUT_TO_SUBMIT
                    && (callback.getEvent() == Event.MOVE_TO_PAYMENT_PENDING
                        || callback.getEvent() == Event.ROLLBACK_PAYMENT
                        || callback.getEvent() == Event.ROLLBACK_PAYMENT_TIMEOUT
                        || callback.getEvent() == Event.ROLLBACK_PAYMENT_TIMEOUT_TO_PAYMENT_PENDING)) {
