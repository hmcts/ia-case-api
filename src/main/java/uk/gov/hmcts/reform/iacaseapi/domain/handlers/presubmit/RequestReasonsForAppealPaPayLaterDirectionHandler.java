...
@Component
public class RequestReasonsForAppealPaPayLaterDirectionHandler implements PreSubmitCallbackHandler<AsylumCase> {
    private final int paPayLaterDueDate;
    private final DateProvider dateProvider;
    private final DirectionAppender directionAppender;

    public RequestReasonsForAppealPaPayLaterDirectionHandler(
            @Value("${paPayLaterDueDate}")
            int paPayLaterDueDate,
    ...
    public boolean canHandle(
        ...
        return callbackStage == PreSubmitCallbackStage.ABOUT_TO_SUBMIT
                && callback.getEvent().equals(Event.REQUEST_REASONS_FOR_APPEAL)
    ...
    public PreSubmitCallbackResponse<AsylumCase> handle(
        ...
        String content = "Your appeal requires a fee to be paid. To avoid the Tribunal taking any action to recover this fee you should pay £" +
                feeAmount +
        ...
        AsylumCase finalCase = HandlerUtils.feeDirectionReminder(asylumCase, directionAppender, dateProvider, paPayLaterDueDate, DirectionTag.REQUEST_REASONS_FOR_APPEAL_PA_PAY_LATER, content);
