...
public class FtpaDisplayService {
    ...
    public void mapFtpaDecision(boolean isMigration, AsylumCase asylumCase, String ftpaApplicantType, FtpaApplications ftpaApplication) {
        ...
        ftpaApplication.setIsFtpaNoticeOfDecisionSetAside(asylumCase
                .read(valueOf("IS_FTPA_%s_NOTICE_OF_DECISION_SET_ASIDE".formatted(ftpaApplicantType)), YesOrNo.class)
                .orElse(YesOrNo.NO));

        String ftpaDecisionOutcomeType = asylumCase.read(
                        valueOf("FTPA_%s_RJ_DECISION_OUTCOME_TYPE".formatted(ftpaApplicantType)), String.class)
        ...
        final Optional<List<IdValue<DocumentWithDescription>>> maybeFtpaDecisionNoticeDocument = asylumCase.read(
                valueOf("FTPA_%s_NOTICE_DOCUMENT".formatted(ftpaApplicantType)));
        final Optional<FtpaDecisionCheckValues<String>> maybeDecisionNotesPoints =
                asylumCase.read(valueOf("FTPA_%s_RJ_DECISION_NOTES_POINTS".formatted(ftpaApplicantType)));
        ...
        asylumCase.read(valueOf("FTPA_%s_DECISION_OBJECTIONS".formatted(ftpaApplicantType)), String.class)
                .ifPresent(ftpaApplication::setFtpaDecisionObjections);
        asylumCase.read(valueOf("FTPA_%s_RJ_DECISION_NOTES_DESCRIPTION".formatted(ftpaApplicantType)), String.class)
                .ifPresent(ftpaApplication::setFtpaDecisionNotesDescription);
        asylumCase.read(valueOf("FTPA_%s_DECISION_DATE".formatted(ftpaApplicantType)), String.class)
        ...
        if (!isMigration) {
            if (ftpaDecisionOutcomeType.equals("remadeRule31") || ftpaDecisionOutcomeType.equals("remadeRule32")) {
                asylumCase.read(valueOf("FTPA_%s_DECISION_REMADE_RULE_32_TEXT".formatted(ftpaApplicantType)),
                                String.class)
            ...
            if (ftpaDecisionOutcomeType.equals("remadeRule32")) {
                asylumCase.read(valueOf("FTPA_%s_DECISION_REMADE_RULE_32".formatted(ftpaApplicantType)), String.class)
                        .ifPresent(ftpaApplication::setFtpaDecisionRemadeRule32);
            }
            asylumCase.read(valueOf("FTPA_%s_DECISION_LST_INS".formatted(ftpaApplicantType)), String.class)
    ...
    private void addRule35Data(AsylumCase asylumCase, String ftpaApplicantType, FtpaApplications ftpaApplication) {
        final Document rule35NoticeDocument =
                asylumCase.read(
                        valueOf("FTPA_R35_%s_DOCUMENT".formatted(ftpaApplicantType)), Document.class)
                        .orElse(null);

        final Optional<List<IdValue<DocumentWithDescription>>> maybeFtpaR35DecisionNoticeDocument =
                asylumCase.read(valueOf("FTPA_%s_R35_NOTICE_DOCUMENT".formatted(ftpaApplicantType)));
        ...
        asylumCase.read(valueOf("FTPA_%s_R35_LISTING_ADDITIONAL_INS".formatted(ftpaApplicantType)), String.class)
                .ifPresent(ftpaApplication::setFtpaDecisionLstIns);
        asylumCase.read(valueOf("FTPA_%s_R35_DECISION_OBJECTIONS".formatted(ftpaApplicantType)), String.class)
                .ifPresent(ftpaApplication::setFtpaDecisionObjections);
    }
    ...
    private void addFtpaDecisionAndReasons(boolean isMigration, AsylumCase asylumCase,
        ...
        if (isMigration) {
            final Optional<List<IdValue<DocumentWithDescription>>> maybeFtpaDecisionAndReasonsDocument = asylumCase.read(
                    valueOf("FTPA_%s_DECISION_DOCUMENT".formatted(ftpaApplicantType)));
            ...
            final Document ftpaDecisionDocument =
                    asylumCase.read(
                            valueOf("FTPA_APPLICATION_%s_DOCUMENT".formatted(ftpaApplicantType)), Document.class)
                            .orElse(null);
