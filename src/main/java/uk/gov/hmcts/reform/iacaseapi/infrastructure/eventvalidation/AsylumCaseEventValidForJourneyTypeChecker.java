...
public class AsylumCaseEventValidForJourneyTypeChecker implements EventValidChecker<AsylumCase> {
    ...
    public EventValid check(Callback<AsylumCase> callback) {
        ...
        if (aipOnlyEvent.contains(event) && !AIP.equals(journeyType)) {
            log.error("[%s] is invalid for case id [%s] the hearing must be submitted by an appellant to handle this event.".formatted(event, callback.getCaseDetails().getId()));
        ...
        if (reppedOnlyEvent.contains(event) && !REP.equals(journeyType)) {
            log.error("[%s] is invalid for case id [%s] the hearing must be submitted by a representative to handle this event.".formatted(event, callback.getCaseDetails().getId()));
