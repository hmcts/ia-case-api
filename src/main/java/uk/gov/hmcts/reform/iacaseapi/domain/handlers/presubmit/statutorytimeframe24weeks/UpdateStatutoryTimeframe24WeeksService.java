...
public class UpdateStatutoryTimeframe24WeeksService {
    ...
    public AsylumCase updateAsylumCase(AsylumCase asylumCase, YesOrNo newStatutoryTimeframe24WeeksStatus) {
        ...
        boolean statusHasChanged = existingStatutoryTimeframe24Weeks
            .map(existing -> !existing.getCurrentStatusAutoGenerated().equals(newStatutoryTimeframe24WeeksStatus))
            .orElse(true);

        if (statusHasChanged) {
            StatutoryTimeframe24Weeks updatedStatutoryTimeframe24Weeks =
                buildNewStatutoryTimeframe24Weeks(newStatutoryTimeframe24WeeksStatus, statutoryTimeframe24WeeksReason, userDetails, existingStatutoryTimeframe24Weeks);
        ...
        boolean matchingCaseNoteAlreadyExists = existingCaseNotes.orElse(emptyList()).stream()
            .anyMatch(caseNoteIdValue -> {
                CaseNote caseNote = caseNoteIdValue.getValue();
                return caseNote.getCaseNoteSubject().contains(newStatutoryTimeframe24WeeksStatus.toString())
                    && caseNote.getCaseNoteDescription().equals(statutoryTimeframe24WeeksReason);
            });
        
        if (!matchingCaseNoteAlreadyExists) {
            List<IdValue<CaseNote>> allCaseNotes = caseNoteAppender.append(
                buildNewCaseNote(newStatutoryTimeframe24WeeksStatus, statutoryTimeframe24WeeksReason, userDetails), existingCaseNotes.orElse(Collections.emptyList()));
            asylumCase.write(CASE_NOTES, allCaseNotes);
        } else {
            log.info("Skipping case note creation as a matching one already exists");
        }
