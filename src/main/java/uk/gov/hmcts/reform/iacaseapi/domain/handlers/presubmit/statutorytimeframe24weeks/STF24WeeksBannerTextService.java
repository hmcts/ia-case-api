...
public class STF24WeeksBannerTextService {
    ...
    public STF24WeeksBannerTextService(BannerTextService bannerTextService) {
        this.bannerTextService = bannerTextService;
    }

    public void updateBannerText(AsylumCase asylumCase) {

        final String stf24wBannerText = populateSTF24wBannerText(asylumCase);
        asylumCase.read(APPEAL_SUBMISSION_DATE);
        Optional<YesOrNo> existingStatus = asylumCase.read(STF_24W_CURRENT_STATUS_AUTO_GENERATED, YesOrNo.class);
        if (existingStatus.isEmpty()) {
            throw new IllegalStateException("stf24wCurrentStatusAutoGenerated is not present");
        } else {
            if (existingStatus.get().equals(YES)) {
                bannerTextService.addToBannerText(asylumCase, stf24wBannerText);
            } else {
                bannerTextService.removeFromBannerText(asylumCase, stf24wBannerText);
            }
        }
    }
